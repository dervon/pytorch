...
bool extractOptionalBNParams(
    const script::Module& bn,
    ConvBNParameters& r) {
  auto eps_weight_bias_constants = findEPSWeightBiasConstantsOfBN(bn);

  if (bn.hasattr("eps")) {
    r.bn_eps = bn.attr("eps").toDouble();
  } else {
    auto optional_eps = eps_weight_bias_constants[0];
    if(!optional_eps) {
      return false;
    }
  ...
  if (bn.hasattr("weight")) {
    ...
    auto optional_bn_weight = eps_weight_bias_constants[1];
  ...
  if (bn.hasattr("bias")) {
    ...
    auto optional_bn_bias = eps_weight_bias_constants[2];
    if (!optional_bn_bias) {
      return false;
    }
